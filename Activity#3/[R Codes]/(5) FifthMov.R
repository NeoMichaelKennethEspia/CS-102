...
url5 <- 'https://www.imdb.com/title/tt4154756/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrape_page <- function(url5, PaginationKey) {
...
for (page in 1:pages_to_scrape) {
  scraped_data <- scrape_page(url5, PaginationKey)
...
DataFrame <- data.frame(
  Title = rep("Avengers: Infinity War", reviews_to_scrape),
...
write.csv(DataFrame, file = "5FifthMov.csv", row.names = FALSE)
print(DataFrame)
