...
url4 <- 'https://www.imdb.com/title/tt10872600/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrape_page <- function(url4, PaginationKey) {
...
for (page in 1:pages_to_scrape) {
  scraped_data <- scrape_page(url4, PaginationKey)
...
DataFrame <- data.frame(
  Title = rep("Spider-Man: No Way Home", reviews_to_scrape),
...
write.csv(DataFrame, file = "4FourthMov.csv", row.names = FALSE)
print(DataFrame)
