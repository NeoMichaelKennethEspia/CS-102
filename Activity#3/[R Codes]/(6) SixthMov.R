...
url6 <- 'https://www.imdb.com/title/tt10545296/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrape_page <- function(url6, PaginationKey) {
...
for (page in 1:pages_to_scrape) {
  scraped_data <- scrape_page(url6, PaginationKey)
...
DataFrame <- data.frame(
  Title = rep("The Hunger Games: The Ballad of Songbirds & Snakes", reviews_to_scrape),
...
write.csv(DataFrame, file = "6SixthMov.csv", row.names = FALSE)
print(DataFrame)
