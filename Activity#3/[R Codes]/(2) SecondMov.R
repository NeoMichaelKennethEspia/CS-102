...
url2 <- 'https://www.imdb.com/title/tt5308322/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrape_page <- function(url2, PaginationKey) {
...
for (page in 1:pages_to_scrape) {
  scraped_data <- scrape_page(url2, PaginationKey)
...
DataFrame <- data.frame(
  Title = rep("Happy Death Day", reviews_to_scrape),
...
write.csv(DataFrame, file = "2SecondMov.csv", row.names = FALSE)
print(DataFrame)
