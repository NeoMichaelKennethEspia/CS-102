...
url3 <- 'https://www.imdb.com/title/tt5886046/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrape_page <- function(url3, PaginationKey) {
...
for (page in 1:pages_to_scrape) {
  scraped_data <- scrape_page(url3, PaginationKey)
...
write.csv(DataFrame, file = "3ThirdMov.csv", row.names = FALSE)
print(DataFrame)
